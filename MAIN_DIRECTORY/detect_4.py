...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            crossingLine = [[(640, 1405), (1321, 52)],[(1489,1405), (1512,32)],[(1669, 99), (2229, 1369)]]
            ...
            if len(outputs):
                ...
                # for *xyxy, conf, cls in reversed(det):
                    ...
                    xyxy_bike = torch.tensor([x-20,y-100,w,h])
                    if(1 and names[int(cls)] == "bike" and h > im0.shape[1]/2):
                        ...
                        cls2 = run_helmet(weights='weights/helmet_3.pt', source=save_dir / 'crops' / names[int(cls)] / f'{p.stem}.jpg')
                        print("helmet classes : ", cls2)
                        # cv2.waitKey(1000)
                    if(1 and names[int(cls)] != "bike" or 1 and h > im0.shape[1]/2):
                        ...
                        plate_path = save_dir / 'crops' / names[int(cls)] / f'{p.stem}.jpg'
                        if(plate_cnt > 1):
                            plate_path = str(save_dir / 'crops' / names[int(cls)] / f'{p.stem}')
                            plate_path += str(plate_cnt)+'.jpg'
                        
                        print("Plate path ======> :", plate_path)
                        # p_x, p_y, p_w, p_h = int(xyxy_plate[0]), int(xyxy_plate[1]), int(xyxy_plate[2]), int(xyxy_plate[3])
                        cls3, xyxy_plate = run_plate(weights='weights/plate2.pt', cnt = plate_cnt, save_crop=True, view_img=True,source=plate_path)
                        plate_cnt = plate_cnt + 1
                        cv2.waitKey(100)
                        # if xyxy_plate:
                        #     p_x, p_y, p_w, p_h = int(xyxy_plate[0]), int(xyxy_plate[1]), int(xyxy_plate[2]), int(xyxy_plate[3])
                        #     xyxy_plate = [p_x, p_y, p_w, p_h]
                        #     print("xyxy plate : ",xyxy_plate)
                        #     save_one_box(xyxy_plate, imc, file=save_dir / 'crops' / names[int(cls)] / f'{p.stem}.jpg', BGR=True)
                        #     print("plate classes : ", cls3)
                        #     cv2.waitKey(1000)
