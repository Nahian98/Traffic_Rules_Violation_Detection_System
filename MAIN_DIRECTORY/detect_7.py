...
def run(
    ...
    unauthorized_vehicle_plate = [0]*100000

    helmet_violated = [0]*100000

    speed_violated = [0]*100000
    speed_violated_plate = [0]*100000

    lane_violation = [0]*100000
    lane_violation_plate = [0]*100000
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(outputs):
                ...
                # for *xyxy, conf, cls in reversed(det):
                    ...
                    # h = int(h)
                    # print("ex1")
                    annotate_name = names[int(cls)]+str(id)+": "
                    vv = 0
                    ...
                    #     center = ((x,int((y+h)/2)), (w,int((y+h)/2)) )
                    ...
                    #             pos_value_0_0 = get_value(crossingLine[0][0], crossingLine[0][1],pts[id][gg-1][0])
                    #             pos_value_0_0 = int(pos_value_0_0)
                    #             pos_value_0_1 = get_value(crossingLine[0][0], crossingLine[0][1],pts[id][gg-1][1])
                    #             pos_value_0_1 = int(pos_value_0_1)
                    #             if(pos_value_0_0 > 0 and pos_value_0_1 > 0):
                    #                 prev[id][0] = 1
                    #                 # print("pos_value > 0 : {} > {} with id: {}".format(pos_value, 0, id))
                    #             if(pos_value_0_0 <= 0 and pos_value_0_1 <= 0):
                    #                 post[id][0] = 1
                    ...
                    #             pos_value_1_0 = get_value(crossingLine[1][0], crossingLine[1][1],pts[id][gg-1][0])
                    #             pos_value_1_0 = int(pos_value_1_0)
                    #             pos_value_1_1 = get_value(crossingLine[1][0], crossingLine[1][1],pts[id][gg-1][1])
                    #             pos_value_1_1 = int(pos_value_1_1)
                    #             if(pos_value_1_0 > 0 and pos_value_1_1 > 0):
                    #                 prev[id][1] = 1
                    #                 # print("pos_value > 0 : {} > {} with id: {}".format(pos_value, 0, id))
                    #             if(pos_value_1_0 <= 0 and pos_value_1_1 <= 0):
                    #                 post[id][1] = 1
                    ...
                    #             pos_value_2_0 = get_value(crossingLine[2][0], crossingLine[2][1],pts[id][gg-1][0])
                    #             pos_value_2_0 = int(pos_value_2_0)
                    #             pos_value_2_1 = get_value(crossingLine[2][0], crossingLine[2][1],pts[id][gg-1][1])
                    #             pos_value_2_1 = int(pos_value_2_1)
                    #             if(pos_value_2_0 > 0 and pos_value_2_1 > 0):
                    #                 prev[id][2] = 1
                    #                 # print("pos_value > 0 : {} > {} with id: {}".format(pos_value, 0, id))
                    #             if(pos_value_2_0 <= 0 and pos_value_2_1 <= 0):
                    #                 post[id][2] = 1
                    ...
                    #             # pos_value_1_0 = get_value(crossingLine[1][0], crossingLine[1][1],pts[id][gg-1])
                    #             # pos_value_1_0 = int(pos_value_1)
                    #             # if(pos_value_1 > 0):
                    #             #     prev[id][1] = 1
                    #             #     # print("pos_value > 0 : {} > {} with id: {}".format(pos_value, 0, id))
                    #             # if(pos_value_1 <= 0):
                    ...
                    #             # pos_value_2_0 = get_value(crossingLine[2][0], crossingLine[2][1],pts[id][gg-1])
                    #             # pos_value_2_0 = int(pos_value_2)
                    #             # if(pos_value_2 > 0):
                    #             #     prev[id][2] = 1
                    #             #     # print("pos_value > 0 : {} > {} with id: {}".format(pos_value, 0, id))
                    #             # if(pos_value_2 <= 0):
                    ...
                    #             thickness = int(np.sqrt(2 / float(i + 1)) * 1.5)
                    #             line_center = (int((pts[id][gg][0][0]+pts[id][gg][1][0])/2), pts[id][gg][0][1])
                    #             line_center2 = (int((pts[id][gg-1][0][0]+pts[id][gg-1][1][0])/2), pts[id][gg-1][0][1])
                    #             cv2.line(im0, line_center, line_center2, (0, 0, 255), thickness)

                    #             # cv2.line(im0, pts[id][gg - 1][0], pts[id][gg][0], (0, 0, 255), thickness)
                    #             # cv2.line(im0, pts[id][gg - 1][1], pts[id][gg][1], (0, 0, 255), thickness)
                    #             thick_len -= 1
                    ...
                    if(names[int(cls)] == 'motorcycle' and x > 132):
                        if(helmet_violated[id] == 0):
                            cv2.imwrite("RESULTS/cropped_vehicles/im_{}_bike.jpg".format(plate_cnt), imc[y-100:int(h-(h-y)/2),x:w])
                        helmet_violated[id] = 1
                        plate_cnt += 1
                        annotator.box_label(torch.tensor([x,y,w,h]), names[int(cls)], color=(0,0,255))
                    ...
                    #             if(y-100 >= 0 and int(h-(h-y)/2) < im_y and helmet_violated[id] == 0):
                    #                 cv2.imwrite("RESULTS/cropped_vehicles/im_{}_bike.jpg".format(plate_cnt), imc[y-100:int(h-(h-y)/2),x:w])
                    #                 hel_path = "RESULTS/cropped_vehicles/im_{}_bike.jpg".format(plate_cnt)
                    #                 cls2 = run_helmet(weights='weights/helmet_3.pt', source=hel_path)
                    #                 if(int(cls2) == 0):
                    #                     helmet_detected[id] = 0;
                    #                     helmet_violated[id] = 1;
                    #             # plate_cnt = plate_cnt + 1
                    #             # plate_cnt = plate_cnt + 1
                    ...
                    #         # cv2.waitKey(1000)
                    #     if(h > im_y-int(im_y/10) and helmet_violated[id] == 1):
                    #         cv2.imwrite("RESULTS/cropped_vehicles/im_{}.jpg".format(plate_cnt), imc[y-100:int(h-(h-y)/2),x-20:w])
                    ...
                    # if(lane_violation[id] == 0 and (post[id][0] == 1 and prev[id][0] == 1) or (post[id][1] == 1 and prev[id][1] == 1) or (post[id][2] == 1 and prev[id][2] == 1)):
                    #     # annotator.box_label(xyxy, vehicle_speed, color=(0,0,255))
                        
                    #     # if(names[int(cls)] != 'bike'):
                    #     #     vx, vy, vw, vh = x,int(y+(h-y)/2+(h-y)/4),w,h
                    #     #     cv2.imwrite("RESULTS/cropped_vehicles/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #     #     cv2.imwrite("RESULTS/cropped_vehicles/im_{}_for_plate.jpg".format(plate_cnt), imc[vy:vh,vx:vw])
                    #     #     plate_path = "RESULTS/cropped_vehicles/im_{}_for_plate.jpg".format(plate_cnt)
                    #     #     print("''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''")
                    #     #     if lane_violation[id] == 0 and run_plate(weights='weights/plate2.pt', cnt = plate_cnt, save_crop=True, view_img=True,source=plate_path):
                    #     #         lane_violation[id] = 1
                    #     #         cv2.imwrite("RESULTS/lane_jump/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #     #         plate_cnt = plate_cnt + 1
                    #     # else:
                    #     #     cv2.imwrite("RESULTS/lane_jump/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #     #     plate_cnt = plate_cnt+1
                    #     lane_violation[id] = 1
                    # if(y > im_y/2):
                    #     if(lane_violation[id] and lane_violation_plate[id] == 0):
                    #         if(names[int(cls)] != 'bike'):
                    #             vx, vy, vw, vh = x,int(y+(h-y)/2+(h-y)/4),w,h
                    #             cv2.imwrite("RESULTS/cropped_vehicles/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #             cv2.imwrite("RESULTS/lane_jump/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    ...
                    #                 t = 1
                    #             else:
                    #                 cv2.imwrite("RESULTS/license_plate/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #         plate_cnt = plate_cnt + 1
                    #         lane_violation_plate[id] = 1
                    #             # else:
                    #             #     cv2.imwrite("RESULTS/cropped_vehicles/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #             #     cv2.imwrite("RESULTS/lane_jump/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #             #     cv2.imwrite("RESULTS/license_plate/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    ...
                    # else:
                    #     if(lane_violation[id] and lane_violation_plate[id] == 0 and names[int(cls)] == 'bike'):
                    #         cv2.imwrite("RESULTS/cropped_vehicles/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #         cv2.imwrite("RESULTS/lane_jump/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #         cv2.imwrite("RESULTS/license_plate/im_{}.jpg".format(plate_cnt), imc[y:h,x:w])
                    #         plate_cnt = plate_cnt + 1
                    ...
                    #     if(over_mid_x >= x-(int(abs(x-w)/100)*50)  and over_mid_x <= w+(int(abs(x-w)/100)*50)  and over_h < h and id != over_id):
                    #     # if(over_h < h and id != over_id):
                    #         overtake_behind[id][over_id]=1
                    # for over_x, over_y, over_w, over_h, over_id, over_cls in reversed(outputs):
                    #     over_mid_x = int((over_x+over_w)/2)
                    #     over_mid_y = int((over_y+over_h)/2)
                    #     if(over_mid_x >= x-(int(abs(x-w)/100)*50) and over_mid_x <= w+(int(abs(x-w)/100)*50) and over_mid_y >= h and over_y <= h and overtake_behind[id].get(over_id) == 1 and id != over_id):
                    ...
                    # if(vv == 1):
                    #     annotator.box_label(torch.tensor([x,y,w,h]),annotate_name, color=(255,0,0))
                    # else:
                    #     annotator.box_label(torch.tensor([x,y,w,h]),annotate_name, color=(0,255,0))
